...
public class KillsAsAssassin extends General{

	public KillsAsAssassin(Clans i) {
		super(i, "Kills As Assassin", new String[]{
				ChatColor.GRAY + "Kill a total of " + ChatColor.GREEN + "10" + ChatColor.GRAY + " players",
				ChatColor.GRAY + "using the" + ChatColor.GREEN + " Assassin " + ChatColor.GRAY + "class."
		});
		...
		if(isActive()){
			...
			if(cl != null){
				if(cl.getDamager() instanceof Player){
					...
					if(Role.getRole(d) != null && Role.getRole(d).getName().equals("Assassin")){
